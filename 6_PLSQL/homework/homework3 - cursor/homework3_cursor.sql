...
BEGIN
  ...
  LOOP
    ...
    dbms_output.put_line(output1.ecnno || ', ' || output1.models || ', ' || output1.title || ', ' ||
                         output1.suppliername || ', ' ||  output1.released_date || ', ' || output1.unitname || ', ' ||
                         output1.survey_vendor_sdate || ', ' || output1.survey_vendor_pdate || ', ' || output1.survey_vendor_edate || ', ' ||
                         output1.survey_status);
  END LOOP;
  CLOSE cursor1;
END;