...
CREATE OR REPLACE PACKAGE PA_EC_Stuff AS
  PROCEDURE fetch_ec_info(input_ecnno            IN VARCHAR2,
...
--package body
CREATE OR REPLACE PACKAGE BODY PA_EC_Stuff AS
    PROCEDURE fetch_ec_info(input_ecnno            IN VARCHAR2,
                            input_vendor           IN VARCHAR2,
                            input_models           IN VARCHAR2,
                            input_release_date_end IN VARCHAR2,
                            input_survey_status    IN NVARCHAR2,
                            input_unitname         IN NVARCHAR2) IS
    ...
    BEGIN
        ...
        ELSE
            LOOP
                v_result := v_result || output1.ecnno || ', ' ||
                            output1.models || ', ' || output1.title || ', ' ||
                            output1.suppliername || ', ' ||
                            output1.released_date || ', ' ||
                            output1.unitname || ', ' ||
