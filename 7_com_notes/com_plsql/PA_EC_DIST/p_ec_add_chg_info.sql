...
--chg要的零件清單，要廠商調查時
CURSOR CUR_SURVEY_PARTS(ECID INT) IS
...
BEGIN
    ...
    FOR i IN 1..3 LOOP
        ...
        IF chgunit <> 'X' THEN
            ls_model := '';
            undertake := '';
            IF chgunit = 'M' OR chgunit = 'I' THEN --M:行銷部  I:INFINITI
                ls_model := 'ALL'; --不分車系承辦
 ...
 --IF chgUnit='M' AND varSectionID<>'YNW0J00' THEN --行銷承辦換科
                    IF chgunit='M' AND varsectionid <> varchgsectionid THEN --行銷承辦換科
                        SELECT
 ...
 --IF chgUnit='I' AND varSectionID<>'YNL0F00' THEN --INFINITI
                    IF chgunit='I' AND varsectionid <> varchgsectionid THEN --INFINITI
                        SELECT
                            f_sys_employinfo_getmail3byno(f_sys_glossary_getnamebyfield('A14-2', 2)) INTO varmail2
                        FROM
                            dual; --資管科
                    ...
                    hadpl := 0;
                    IF vendor_survey='Y' THEN --抓取T_EC_SURVEY_PARTS
 ...
 --END IF;
                    ...
                    IF hadpl = 0 THEN --車系承辦, 無PL, 新增虛擬NA件號(記錄)
                        varpl_id := 0;
                        varcount := 0;
                        SELECT
                            COUNT(*) INTO varcount
...
*/
 ...
 --        OutResult := 'ok';
 --    END IF;
    IF ok_flag = 'Y' THEN --有問題維持A04-1, A04重新分發
        UPDATE t_ec_info e
        SET
            e.status = 'A04-4'
        WHERE
